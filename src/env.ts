 ...
 */
export function getEnvVar(varName: string, defaultValue = ''): string {
 ...
 */
export function getEnvVarAsNumber(varName: string, defaultValue = 0): number {
  const value = process.env[varName]
  if (!value || value.length === 0) {
    return defaultValue
  }
 ...
 */
export function getEnvVarAsInt(varName: string, defaultValue = 0): number {
  const value = process.env[varName]
  if (!value || value.length === 0) {
    return defaultValue
  }
 ...
 */
export function getEnvVarAsBoolean(varName: string, defaultValue = false): boolean {
  const value = process.env[varName]
  if (!value || value.length === 0) {
    return defaultValue
  }
 ...
 */
export function getEnvVarAsArray(varName: string, defaultValue: string[] = []): string[] {
  const value = process.env[varName]
  if (!value || value.length === 0) {
    return defaultValue
  }
